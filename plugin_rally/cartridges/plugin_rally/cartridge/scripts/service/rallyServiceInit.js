...
function rallyService(currService) {
    var rallyServiceObj = LocalServiceRegistry.createService(currService, {
        createRequest: function (svc, params) {
            var isSandbox = Site.getCurrent().getCustomPreferenceValue('rallyScriptSource');
            var serviceUrl = isSandbox ? 'https://salesforce.connector.sandbox.onrally.com' : 'https://salesforce.connector.onrally.com';
            ...
            if (params.action) {
                svc.setURL(serviceUrl + '/api/v1/webhooks/' + params.action);
            }
